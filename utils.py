...
def trip_distance(file, src, dst):
    match = file
...
def shortest_path(start, end, shp_file='taxi_zones.shp',
                  adjacent_path = 'adjacent_zone.csv', 
                  distance_path = 'trip_distance_src_dst.csv'):
    
    match = pd.read_csv(distance_path)
    ...
    ad = pd.read_csv(adjacent_path)
    for i in range(ad.shape[0]):
        s = int(ad.iloc[i,0])
        e = int(ad.iloc[i,1])
        w = trip_distance(match, s, e)
        if (w != -1):
